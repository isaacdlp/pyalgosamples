...
from pyalgoext import dbfeed, iplots
...
# Modificaciones al modulo plotter
...
if __name__ == "__main__":
    ...
    plt1 = plotter.StrategyPlotter(myStrategy, True)
    plt2 = plotter.StrategyPlotter(myStrategy, False, False, False)

    if hasattr(myStrategy, '_aroon'):
        subPlot = plt2.getOrCreateSubplot("Aroon")
        subPlot.addDataSeries("Aroon", myStrategy._aroon[instrument])

    if hasattr(myStrategy, '_smaShort'):
        subPlot = plt2.getOrCreateSubplot("SMA")
        ...
        subPlot = plt2.getOrCreateSubplot("MACD")
    ...
    iplots.plot(plt1.buildFigure(), filename='plot1.html')
    iplots.plot(plt2.buildFigure(), filename='plot2.html')
