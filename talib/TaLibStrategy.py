...
class MyBenchmark(strategy.BacktestingStrategy):
    ...
    def onEnterOk(self, position):
        ...
        if order.getAction() == broker.Order.Action.BUY:
            ...
            self.logOp("VENTA CORTA", position.getEntryOrder())
...
if __name__ == "__main__":
    ...
    myStrategy = None
    if SCENARIO == 2:
        myStrategy = MyBasicStrategy(feed, smaShort, smaLong)
    elif SCENARIO == 3:
        myStrategy = MyTaLibStrategy(feed, smaShort, smaLong, aroonPeriod)
    else:
        myStrategy = MyBenchmark(feed, smaLong)
