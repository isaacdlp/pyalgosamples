...
class MyTaLibStrategy(MyBasicStrategy):
    def __init__(self, feed, stopPer, stopTrailing, smaShort, smaLong, macdPriceShort, macdPriceLong, macdPriceSignal, macdVolShort, macdVolLong, macdVolSignal, aroonPeriod, aroonIn, aroonOut):
        self._active = False
        try:
            MyBasicStrategy.__init__(self, feed, stopPer, stopTrailing, smaShort, smaLong, macdPriceShort, macdPriceLong, macdPriceSignal, macdVolShort, macdVolLong, macdVolSignal)

            self.info("%s %s %s %s %s %s %s %s %s %s %s %s %s" % (
            stopPer, stopTrailing, smaShort, smaLong, macdPriceShort, macdPriceLong, macdPriceSignal, macdVolShort,
            macdVolLong, macdVolSignal, aroonPeriod, aroonIn, aroonOut))

            self._aroon = {}
            self._aroonPeriod = aroonPeriod
            self._aroonIn = aroonIn
            self._aroonOut = aroonOut
            for instrument in feed.getRegisteredInstruments():
...
def parameters_generator():
    ...
    macdPriceShort = range(5, 50)

    macdPriceLong = range(5, 50)

    macdPriceSignal = range(5, 50)

    macdVolShort = range(5, 50)

    macdVolLong = range(5, 50)

    macdVolSignal = range(5, 50)

    aroonPeriod = range(5, 50)

    aroonIn = range(5, 95)

    aroonOut = range(5, 95)

    return itertools.product(stopPer, stopTrailing, smaShort, smaLong, macdPriceShort, macdPriceLong, macdPriceSignal, macdVolShort,
        macdVolLong, macdVolSignal, aroonPeriod, aroonIn, aroonOut)
